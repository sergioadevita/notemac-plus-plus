...
import { SetEditorAction, ClearEditorAction } from '../../../Shared/Helpers/EditorGlobals';

/**
 * useEditorActions - Custom hook containing the action handler function
 *
 * This hook encapsulates the massive switch statement handling all editor actions:
 * - Clipboard operations (cut, copy, paste)
 * - Undo/Redo
 * - Line operations (duplicate, delete, move, transpose)
 * - Text transformations (case changes, trim, sort)
 * - Folding operations
 * - Bookmark operations
 * - Mark operations
 * - Macro operations (delegated to useMacroPlayback)
 * - Hash and encoding operations
 * - JSON operations
 * - AI actions
 *
 * Returns the action handler function for use in useEffect
 */
export function useEditorActions(
  ...
  _updateTabContent: (tabId: string, content: string) => void,
  _monacoReady: boolean,
  macroPlaybackHandler: (action: string, value?: boolean | string | number) => void,
): (action: string, value?: boolean | string | number) => void {
    ...
    (action: string, value?: boolean | string | number): void => {
      ...
      switch (action) {
        ...
        case 'proper-case': {
          const selection = editor.getSelection();
          if (selection && !selection.isEmpty()) {
            editor.executeEdits('proper', [{
              range: selection,
              text: model.getValueInRange(selection).replace(/\w\S*/g, t => t.charAt(0).toUpperCase() + t.substr(1).toLowerCase()),
            }]);
          }
          break;
        }
        ...
        case 'sentence-case': {
          const selection = editor.getSelection();
          if (selection && !selection.isEmpty()) {
            editor.executeEdits('sentence', [{
              range: selection,
              text: model.getValueInRange(selection).toLowerCase().replace(/(^\s*\w|[.!?]\s+\w)/g, c => c.toUpperCase()),
            }]);
          }
          break;
        }
        ...
        case 'invert-case': {
          const selection = editor.getSelection();
          if (selection && !selection.isEmpty()) {
            editor.executeEdits('invert', [{
              range: selection,
              text: model.getValueInRange(selection).split('').map(c => c === c.toUpperCase() ? c.toLowerCase() : c.toUpperCase()).join(''),
            }]);
          }
          break;
        }
        ...
        case 'random-case': {
          const selection = editor.getSelection();
          if (selection && !selection.isEmpty()) {
            editor.executeEdits('random', [{
              range: selection,
              text: model.getValueInRange(selection).split('').map(c => Math.random() > 0.5 ? c.toUpperCase() : c.toLowerCase()).join(''),
            }]);
          }
          break;
        }
        ...
        case 'remove-duplicates': {
          const fullRange = model.getFullModelRange();
          editor.executeEdits('remove-dup', [{
            range: fullRange,
            text: [...new Set(model.getValue().split('\n'))].join('\n'),
          }]);
          break;
        }
        ...
        case 'remove-consecutive-duplicates': {
          const fullRange = model.getFullModelRange();
          const lines = model.getValue().split('\n');
          editor.executeEdits('remove-consec-dup', [{
            range: fullRange,
            text: lines.filter((l, i) => i === 0 || l !== lines[i - 1]).join('\n'),
          }]);
          break;
        }

        case 'remove-empty-lines': {
          editor.executeEdits('remove-empty', [{
            range: model.getFullModelRange(),
            text: model.getValue().split('\n').filter(l => l.trim() !== '').join('\n'),
          }]);
          break;
        }

        case 'remove-blank-lines': {
          editor.executeEdits('remove-blank', [{
            range: model.getFullModelRange(),
            text: model.getValue().split('\n').filter(l => l !== '').join('\n'),
          }]);
          break;
        ...
        case 'tab-to-space': {
          ...
          editor.executeEdits('tab-space', [{
            range: model.getFullModelRange(),
            text: model.getValue().replace(/\t/g, spaces),
          }]);
          break;
        }

        case 'space-to-tab-leading': {
          const tabSz = settings.tabSize;
          const regex = new RegExp(`^( {${tabSz}})+`, 'gm');
          editor.executeEdits('space-tab', [{
            range: model.getFullModelRange(),
            text: model.getValue().replace(regex, (m) => '\t'.repeat(m.length / tabSz)),
          }]);
          break;
        }
        ...
        case 'space-to-tab-all': {
          const tabSz = settings.tabSize;
          editor.executeEdits('space-tab-all', [{
            range: model.getFullModelRange(),
            text: model.getValue().split(' '.repeat(tabSz)).join('\t'),
          }]);
          break;
        }
        ...
        case 'insert-blank-above': {
          const pos = editor.getPosition();
          if (pos) editor.executeEdits('blank-above', [{
        ...
        case 'insert-blank-below': {
          const pos = editor.getPosition();
          if (pos) editor.executeEdits('blank-below', [{
            range: new monaco.Range(pos.lineNumber, model.getLineMaxColumn(pos.lineNumber), pos.lineNumber, model.getLineMaxColumn(pos.lineNumber)),
            text: '\n',
          }]);
          break;
        }
        ...
        // Macro playback (delegated to macro hook)
        case 'macro-playback':
        case 'macro-run-multiple':
        case 'macro-save': {
          macroPlaybackHandler(action, value);
          break;
        }
        ...
        case 'json-format': {
          try {
            editor.executeEdits('json-fmt', [{
              range: model.getFullModelRange(),
              text: JSON.stringify(JSON.parse(model.getValue()), null, 2),
            }]);
          } catch { /* Content is not valid JSON — no-op */ }
          break;
        }

        case 'json-minify': {
          try {
            editor.executeEdits('json-min', [{
              range: model.getFullModelRange(),
              text: JSON.stringify(JSON.parse(model.getValue())),
            }]);
          } catch { /* Content is not valid JSON — no-op */ }
          break;
        }
        ...
        case 'hash-sha512': {
          ...
          computeHash(action.replace('hash-', ''), hashText).then(hash => {
            const pos = editor.getPosition();
            if (pos) editor.executeEdits('hash', [{
              range: new monaco.Range(pos.lineNumber, pos.column, pos.lineNumber, pos.column),
        ...
        case 'hash-sha512-clipboard': {
          ...
          computeHash(action.replace('hash-', '').replace('-clipboard', ''), hashText2)
            .then(hash => navigator.clipboard.writeText(hash))
            .catch(() => { /* Hash or clipboard write failed */ });
          break;
        }
        ...
        case 'base64-decode': {
          ...
          if (sel5 && !sel5.isEmpty()) try {
            editor.executeEdits('b64dec', [{ range: sel5, text: atob(model.getValueInRange(sel5)) }]);
          } catch { /* Invalid base64 — no-op */ }
          break;
        }
        ...
        case 'url-decode': {
          ...
          if (sel7 && !sel7.isEmpty()) try {
            editor.executeEdits('urldec', [{ range: sel7, text: decodeURIComponent(model.getValueInRange(sel7)) }]);
          } catch { /* Malformed URI component — no-op */ }
          break;
        }
...
/**
 * Register the action handler globally
 */
export function registerActionHandler(handler: (action: string, value?: boolean | string | number) => void): void {
  SetEditorAction(handler);
}

/**
 * Clear the action handler globally
 */
