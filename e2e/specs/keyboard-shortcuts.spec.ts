...
test.describe('Keyboard Shortcuts', () => {
  test.beforeEach(async ({ page }) => {
    await gotoApp(page);
    // Ensure the app has focus for keyboard shortcuts
    await focusApp(page);
  });

  // Tab management shortcuts — dispatch directly because Ctrl+N/W are browser-intercepted
  test('Cmd+N creates new tab', async ({ page }) => {
    const countBefore = await getTabCount(page);
    await dispatchShortcut(page, 'Cmd+n');
    await page.waitForTimeout(300);
    const countAfter = await getTabCount(page);
    expect(countAfter).toBe(countBefore + 1);
  });

  test('Cmd+W closes tab', async ({ page }) => {
    await createNewTab(page);
    const countBefore = await getTabCount(page);
    await dispatchShortcut(page, 'Cmd+w');
    await page.waitForTimeout(300);
    const countAfter = await getTabCount(page);
    expect(countAfter).toBe(countBefore - 1);
  });
  ...
  test('Cmd+Shift+T restores closed tab', async ({ page }) => {
    ...
    await pressShortcut(page, 'Cmd+Shift+T');
    await page.waitForTimeout(300);
  ...
  // Find/Replace shortcuts — dispatch directly because Ctrl+F/H are browser-intercepted
  test('Cmd+F opens find panel', async ({ page }) => {
    await dispatchShortcut(page, 'Cmd+f');
    await page.waitForTimeout(300);
  ...
  test('Cmd+H opens replace panel', async ({ page }) => {
    await dispatchShortcut(page, 'Cmd+h');
    await page.waitForTimeout(300);
  ...
  // View shortcuts — dispatch directly because Ctrl+B is browser-intercepted
  test('Cmd+B toggles sidebar', async ({ page }) => {
    const visibleBefore = await isSidebarVisible(page);

    await dispatchShortcut(page, 'Cmd+b');
    await page.waitForTimeout(300);
  ...
  // Zoom shortcuts - now using real keyboard shortcuts
  test('Cmd+= increases zoom', async ({ page }) => {
  ...
  test('Cmd+- decreases zoom', async ({ page }) => {
    // First increase zoom
    await pressShortcut(page, 'Cmd+=');
    await page.waitForTimeout(200);
    await pressShortcut(page, 'Cmd+=');
  ...
  // Zoom accumulation via keyboard shortcuts
